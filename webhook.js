...
function dateDiff(datepart, fromdate, todate) {	
  datepart = datepart.toLowerCase();	/* exported Script */
/* globals console, _, s */
...
class Script {
  ...
  if (myContent.end.dateTime) {
    ...
    var myText = myContent.organizer.displayName ?? "" + " Google Calendar Event \n";
    ...
    if (myContent.location) {
      myText = myText + 'Local:      *' + myContent.location + '*\n';     
    }

    myText = myText + 'Início:    *' + dateStartText +'*\n';    	
	  myText = myText + 'Fim:    *' + dateEndtText +'*\n';     
    myText = myText + 'Duração:      *' + durationText + ' * \n';
