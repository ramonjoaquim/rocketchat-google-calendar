...
# the url where the webhook should be sent.You can find it when you create# an incoming webhook(after you press save) in rocketchat - > Administration - > integrations
webhook_url = os.environ['WEBHOOK_URL']

# the timezone in which you want the events to appear.Example: "Europe/Athens"
or "Europe/Amsterdam"
or "Asia/Tokyo"
...
If nothing has been stored, or
if the stored credentials are invalid,
...
print('now: ', now)# print('now_plus: ', now_plus)# print('Getting the upcoming events')
...
# read the list with the events that we have already sent# first check
if the file exists
...
# Define an empty list to put the events you just received# this will be saved as your new list so that you dont ever sent the same event twice
sentEventsNew = []

for event in events:
  sentEventsNew.append(event['id'])
if event['id'] not in sentEvents: #uncomment the next two lines
if you want to debug it in shell / terminal# start = event['start'].get('dateTime', event['start'].get('date'))# print(start, event['summary'])
payload = json.dumps(event)# POST with form - encoded data
